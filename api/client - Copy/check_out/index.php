	...
	while($row=mysqli_fetch_row($result)) 
	{
		...
		if($dist<=0){
			...
			if(isset($_POST['item_image'])){
				...
				$q="Update attendence set out_time = '".mysqli_escape_string($cn,$time)."', out_image = '".mysqli_escape_string($cn,$PdfName)."', out_latitude = '".mysqli_escape_string($cn,$_POST['latitude'])."', out_longitude = '".mysqli_escape_string($cn,$_POST['latitude'])."' where staff_uid = '".mysqli_escape_string($cn,$_POST['staff_uid'])."' AND date = '".mysqli_escape_string($cn,$date)."'";
				$r=mysqli_query($cn,$q);
				if($r){
